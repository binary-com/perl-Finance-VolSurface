package Finance::VolSurface::Moneyness;
...
sub _market_maturities_interpolation_function {
    ...
    my $tau1 = $self->weighted_days_in_period($dates{T1}, $dates{T}) / 365;
    my $tau2 = $self->weighted_days_in_period($dates{T1}, $dates{T2}) / 365;
...
sub get_smile_expiries {
    ...
    my $effective_date = $self->effective_date;
    my $close = $self->calendar->standard_closing_on($self->underlying->exchange, $effective_date);
